...
use fluxion_message::{Message, handler::MessageHandler};
...
impl<AP: ActorParams<S>, S: SystemParams> ActorRef<AP, S> {
    ...
    pub async fn request(
        ...
        // Create the handler
        let (handler, responder) = MessageHandler::new(message);

        // Send the handler
        let res = self.messages.send_async(SupervisorMessage::Message(handler)).await;
        res.or(Err(MessageError::SendError))?;

        // Wait for a response
        let res = responder.await.or(Err(MessageError::ResponseFailed))?;
