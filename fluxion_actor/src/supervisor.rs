...
use fluxion_types::atg::{ActorParams, SystemParams};
use crate::Channel;
use crate::actor_ref::ActorRef;
use fluxion_message::handler::MessageHandler;

use fluxion_types::atg::MessageParams;

#[cfg(all(serde, foreign))]
use fluxion_message::serializer::MessageSerializer;

#[cfg(foreign)]
use fluxion_message::foreign::ForeignMessage;
...
impl<AP: ActorParams<S>, S: SystemParams> ActorSupervisor<AP, S> {
    ...
    async fn handle_foreign(&mut self, foreign: Result<ForeignMessage, flume::RecvError>) {
        ...
        // Decode it
        #[cfg(federated)]
        let decoded = message.decode::<(AP::Message, <S::SystemMessages as MessageParams>::Federated), S::Serializer>();
        #[cfg(not(federated))]
        let decoded = message.decode::<AP::Message, S::Serializer>();
        ...
        match decoded {
            fluxion_message::foreign::ForeignType::Message(m) => {
                // Dispatch
            ...
            #[cfg(federated)]
            fluxion_message::foreign::ForeignType::Federated(m) => {
                // Dispatch
                let res = self.actor.dispatch(&m).await;
