...
pub trait HandleNotification<N: Notification>: Actor {
    ...
    async fn notified<F: Message>(&mut self, context: &mut ActorContext<F, N>, notification: N) -> Result<(), ActorError>;
}
...
pub trait HandleFederated<F: Message>: Actor {
    ...
    async fn federated_message<N: Notification>(&mut self, context: &mut ActorContext<F, N>, federated_message: F) -> Result<F::Response, ActorError>;
}
...
pub trait HandleMessage<M: Message>: Actor {
    ...
    async fn message<F: Message, N: Notification>(&mut self, context: &mut ActorContext<F, N>, message: M) -> Result<M::Response, ActorError>;
}