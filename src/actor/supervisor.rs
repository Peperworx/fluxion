...
pub struct ActorSupervisor<G: SupervisorGenerics> {
    ...
    /// The message channel
    messages: flume::Receiver<MessageHandler<G::Message>>,
}

impl<G: SupervisorGenerics> ActorSupervisor<G> {
    pub fn new(actor: G::Actor) -> (Self, ActorRef<G::Message, <G::Actor as Actor>::Error>) {
        ...
        let reference = ActorRef { message_sender };
    ...
    #[cfg(foreign)]
    pub async fn dispatch_foreign(
        &mut self,
        message: Vec<u8>,
    ) -> Result<Vec<u8>, FluxionError<<G::Actor as Actor>::Error>>
    where
        G::Actor: Handle<G::Message>,
        G::Message: for<'a> serde::Deserialize<'a>,
        <G::Message as Message>::Response: serde::Serialize,
    {
        // Deserialize the message into Foreign
        let message: G::Message = G::Serializer::deserialize(message)?;
    ...
    pub async fn dispatch<R, M, T>(
        &mut self,
        message: &T,
    ) -> Result<T::Response, FluxionError<<G::Actor as Actor>::Error>>
    where
        G::Actor: Handle<M>,
        ...
        T: Message,
    {
        ...
        res.try_into()
            .or(Err(FluxionError::ResponseConversionError))
    }
